...
func GetStudentAveragePtk(SmartBtwID int) []bson.M {
	return []bson.M{
		{
			"$match": bson.M{
				"$expr": bson.M{
...
func GetStudentLastScore(SmartBtwID int) []bson.M {
	return []bson.M{
		...
		{"$unwind": bson.M{"path": "$history_ptk", "preserveNullAndEmptyArrays": true}},
	}
}

func GetLast10StudentScore(SmartBtwID int) []bson.M {
	return []bson.M{
		{"$sort": bson.M{"created_at": 1}},
		{
			"$match": bson.M{
...
func GetStudentPTKHistoryScores(SmartBTWID int, params *request.HistoryPTKQueryParams) []bson.M {
	pipelines := []bson.M{
		{
			"$match": bson.M{
...
func GetRecordOnlyStage() []bson.M {
	return []bson.M{
		{
			"$match": bson.M{
				"$expr": bson.M{
...
func GetNotDeletedStuff() []bson.M {
	return []bson.M{
		{
			"$match": bson.M{
				"$expr": bson.M{
